 ...
 * @version  2.0.0
 */
class WC_Gateway_CCPayment extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->icon               = apply_filters( 'woocommerce_ccpayment_icon', CCPAYMENT_PLUGIN_URL . 'assets/ccpayment.png' );
        ...
        $ccpaymentSettings = get_option('woocommerce_ccpayment_settings', []);
        $ccpaymentSettings['icon'] = $this->icon;
        update_option('woocommerce_ccpayment_settings', $ccpaymentSettings);
...
//            'cancel_url' => $order->get_cancel_order_url(),
        ...
        $message = __( 'Error occurred while processing the payment:  ' . $response['msg'].' '.$response['reason'], 'woocommerce-gateway-ccpayment' );
        infof($message);
    ...
    private function verifyCallbackData($server, $body): bool
    {
        ...
        $appId = sanitize_text_field($server['HTTP_APPID']);
        $timestamp = sanitize_text_field($server['HTTP_TIMESTAMP']);
        $sign = sanitize_text_field($server['HTTP_SIGN']);
    ...
    public function payment_callback(): void
    {
        try {
            ...
            if (!$this->verifyCallbackData($_SERVER, $body)) {
            ...
            if ( !$order  || $order->get_payment_method() != CCPAYMENT_NAME) {
                throw new Exception('This ' . $orderId . '  is not Ccpayment order');
            }
            ...
            switch ($status) { // todo The current states are only two: success or failure.
                case $this->ccpayment::PayStatusNO:
                    WC()->mailer()->emails['WC_Email_New_Order']->trigger($order->get_id());
                    break;
                case $this->ccpayment::PayStatusUnderpaid:
                    WC()->mailer()->emails['WC_Email_Customer_Processing_Order']->trigger($order->get_id());
                    $order->update_status($wcOrderStatus);
                    $order->add_order_note($this->ccpayment::PayStatusUnderpaid);
                    break;
                case $this->ccpayment::PayStatusSuccess:
                    $order->update_status($wcOrderStatus);
                    $order->add_order_note("Paid success");
                  //  $order->payment_complete();
                    break;
                case $this->ccpayment::PayStatusOverpaid:
                    $order->update_status($wcOrderStatus);
                    $order->add_order_note($this->ccpayment::PayStatusOverpaid);
                    break;
                case $this->ccpayment::PayStatusExpired:
                    $order->update_status($wcOrderStatus);
                    $order->add_order_note($this->ccpayment::PayStatusExpired);
                    break;
                case $this->ccpayment::PayStatusFailed:
                    $order->update_status($wcOrderStatus);
                    $order->add_order_note($this->ccpayment::PayStatusFailed);
                    break;
                case $this->ccpayment::PayStatusExpiredUnderpaid:
                    $order->update_status($wcOrderStatus);
    ...
    {
        return array(
            $this->ccpayment::PayStatusNO=> 'Pending',
            $this->ccpayment::PayStatusSuccess => 'Paid',
            $this->ccpayment::PayStatusUnderpaid => 'Underpaid',
            $this->ccpayment::PayStatusOverpaid => 'Overpayed',
            $this->ccpayment::PayStatusFailed => 'Failed',
            $this->ccpayment::PayStatusExpired => 'Expired',
            $this->ccpayment::PayStatusExpiredUnderpaid => 'Expired(Underpaid)',
            //'cancelled' => 'Cancelled'
        );
    }
    ...
    {
        return array(
            $this->ccpayment::PayStatusNO => 'wc-pending',
            $this->ccpayment::PayStatusSuccess => 'wc-completed',
            $this->ccpayment::PayStatusUnderpaid => 'wc-pending',
            $this->ccpayment::PayStatusOverpaid => 'wc-completed',
            $this->ccpayment::PayStatusFailed => 'wc-failed',
            $this->ccpayment::PayStatusExpired => 'wc-cancelled',// $wcStatus == 'wc-cancelled'
            $this->ccpayment::PayStatusExpiredUnderpaid => 'wc-failed',
        );
    }
    ...
    {
        $status = $this->ccpayment::PayStatusNO;
        $paidAmount = "0";
        ...
        if ( $comp == 0) {
            return $this->ccpayment::PayStatusSuccess;
        }
        if ( $comp > 0) {
            return $this->ccpayment::PayStatusOverpaid; // PayStatusOverpaid
        }

        if ( $data['expiredAt'] < time() && $isPaid <= 0){
            return $this->ccpayment::PayStatusExpired; // PayStatusExpired
        }

        if ( $data['expiredAt'] < time() && $isPaid > 0){
            return $this->ccpayment::PayStatusExpiredUnderpaid; // PayStatusExpiredUnderpaid
        }

        if ( $data['expiredAt'] > time() && $isPaid > 0){
            return $this->ccpayment::PayStatusUnderpaid; // PayStatusUnderpaid
        }
        return $status;
    }
